...
def main():
    ...
    # ADDED - try to get cookies using incorporated code from decryptcookies and the parameters we already obtained
    decryptcookies(profile + "\cookies.sqlite", profile + "\plaincookies.txt")
    
# ADDED - decrypt cookies code
...
def decryptcookies(sqldb, destfile):
    # convert sys argv into module parameters    
    #sqldb=sys.argv[1]
    #destfile=sys.argv[2]
    ...
    with open(destfile) as fp:
        for line in fp:
            print line
# END OF ADDENUM - decrypt cookies code
